...
namespace AdvSpareAuto.Controllers
{
    ...
    public class offers
    {
        ...
        [XmlElement("offer")]
        public List<CarAdvXmlFeed> offer { get; set; }
    }
    ...
    public class AdvController : Controller
    {
        ...
        public void XmlFeed()
        {
            ...
            string xml;
            auto_catalog ac = new auto_catalog() { offers = new offers(), creation_date = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " GMT+2", host = "adv.spare-auto.com" };

            var tmp = _advRepository.GetAll().ToList();
            ac.offers.offer = new List<CarAdvXmlFeed>();

            using (var _advContext = new AdvContext())
            {
                var ImgIds =
                    _advContext.Database.SqlQuery<AdvPhoto>("select * from dbo.advPhoto").ToArray();

                string modelName;

                tmp.ForEach(x =>
                {
                    modelName = AdvRepository._ModelPairs.FirstOrDefault(y => y.Id == x.Model) == null
                        ? x.Name
                        : AdvRepository._ModelPairs.FirstOrDefault(y => y.Id == x.Model).Name;

                    ac.offers.offer.Add(new CarAdvXmlFeed()
                    {

                        type = x.type == 2 ? "private" : "commercial",
                        year = x.Year,
                        run = x.MileAge,
                        run_metric = "км",
                        seller_phone = x.SellerPhone,
                        currency_type = "RUB",
                        date = x.Created.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss") + " GMT+2",
                        displacement = "1.6",
                        steering_wheel = "Левый",
                        seller_city = AdvRepository._locations.FirstOrDefault(y => y.CityId == x.Location).Name,
                        mark =
                            AdvRepository._BrandsPairs.FirstOrDefault(y => y.Id == x.Brand) == null
                                ? x.Name
                                : AdvRepository._BrandsPairs.FirstOrDefault(y => y.Id == x.Brand).Name,
                        model = string.IsNullOrEmpty(modelName) ? "none" : modelName,
                        url = "http://adv.spare-auto.com/Adv/AdvDetails/" + x.Id,
                        murl = "http://adv.spare-auto.com/Adv/AdvDetails/" + x.Id,
                        image =
                            "http://adv.spare-auto.com/Photo/Index/" +
                            (ImgIds.FirstOrDefault(y => y.AdvId == x.Id) == null
                                ? "http://adv.spare-auto.com/Images/NoPhotoAuto.jpg"
                                : ImgIds.FirstOrDefault(y => y.AdvId == x.Id).PhotoId.ToString()),
                        stock = "в наличии",
                        price = x.Price
                    });
                });
                ...
                using (StringWriterWithEncoding sww = new StringWriterWithEncoding(UTF8Encoding.UTF8))
                using (XmlWriter writer = XmlWriter.Create(sww, settings))
                {
                    xsSubmit.Serialize(writer, ac, names);
